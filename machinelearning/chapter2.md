...
이 그림을 좀 더 자세하게 설명하면 다음과 같습니다.

1. 현재 θ 값에서 경사값(∇)을 구합니다.

2. 경사를 따라 η 만큼 내려갑니다. (즉, 경사값에 -1을 곱한 방향으로 η 만큼 움직입니다.) θ<sub>step t+1</sub> = θ<sub>step t</sub> - η∇

3. 손실함수의 출력값이 많이 줄어들었는지 보고 그렇다면 1로 돌아가 계속 하강합니다. 많이 줄어들지 않았다면 경사를 다 내려온 것이므로 θ 값 구하기를 그만 둡니다.

여기서 경사값과 학습률을 어떻게 조절할 것인가에 대해서 알아보겠습니다.

수학적으로 경사값은 함수의 1차 미분값이 됩니다.

![그림](images/pic2_10.png)

위 그림은 θ=4 에서의 경사를 보여줍니다. 경사는 곡선의 접선 방향으로 결정되는데, 이를 계산하려면 손실함수를 인자 θ에 대해 미분하면 됩니다.

![그림](images/pic2_11.png)

이 식에 θ=4를 대입하면 경사값은 2*(4-2)=4입니다. 이 경사값을 이용해 앞에서 서술한 과정 1, 2, 3을 반복하면 손실함수를 쉽게 최적화할 수 있습니다. 

경사는 방향을 결정합니다. 얼마나 빨리 내려가는지는 학습률 <sup>learning rate</sup> 혹은 스텝크기<sup>step size</sup>라고 불리는 η에 의해 결정됩니다. 학습률이 크면 매 step마다 Θ를 많이 변화시켜서 빠른 시간에 학습되기는 하지만 최솟값 근처에서 완전히 수렴 하지 못하고 값이 계속 왔다갔다 하는 안정적이지 못하는 모습을 보입니다. 반면 적게 변화시키면 오랜 시간에 걸쳐 학습되지만 최솟값 근처에서는 안정적으로 수렴합니다.
